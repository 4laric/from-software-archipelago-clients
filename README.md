...
### Streaming the overlay

OBS **Game Capture** can miss the client overlay even though it is visible in
game. In the Game Capture source properties, enable **Capture third-party
overlays**. If the overlay is still absent, start Elden Ring and the client
first, then restart OBS or remove and recreate the Game Capture source so OBS
hooks the already-running game.

If Game Capture remains unreliable, use **Display Capture**. It captures the
final desktop image and therefore includes the overlay. This only affects what
OBS records; it does not indicate a problem with checks, item delivery, or the
client connection.
