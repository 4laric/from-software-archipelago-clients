...
**Consequence, stated out loud: a region the seed does not wire is left vanilla.** Unkept and sealed
regions are still physically walkable, so this is reachable in ordinary play, and with `num_regions`
it is common. Making no statement about a region the seed does not cover is the point.
