...
impl Sim {
    fn new(mode: i32) -> Self {
        Sim { mode, stored: 0, frags: 0, floor: 0, bag_ok: true, writes: vec![] }
    }
    ...
    fn tick(&mut self, policy: Policy) {
        ...
        let target = match policy {
            ...
            Policy::FragmentsOnly => {
                if self.mode == 0 { None } else { Some(level_for_fragments(self.frags)) }
            }
        };
...
fn replay(mode: i32, events: &[Ev], policy: Policy) -> Sim {
    ...
    for e in events {
        match *e {
            Ev::Fragments(n) => { s.frags = n; s.bag_ok = true; }
...
fn dlc_region_with_no_fragments_is_floored_not_left_at_zero() {
    ...
    assert_eq!(broken.stored, 0, "pre-fix reproduction: floor ignored, player arrives at blessing 0");

    let fixed = replay(2, &evs, Policy::FloorComposed);
    assert_eq!(fixed.stored, 12, "the DLC floor must lift a fragment-less player to the area's expectation");
}
...
fn collected_fragments_still_count_above_the_floor() {
    ...
    let s = replay(2, &[Ev::EnterRegion(5), Ev::Fragments(26)], Policy::FloorComposed);
    assert_eq!(s.stored, level_for_fragments(26));
    assert!(s.stored > 5, "the floor must not cap a player who earned more");
}

/// Leaving the DLC (floor -> 0) must NOT lower a blessing already granted. Raise-only.
#[test]
fn leaving_the_dlc_never_lowers_the_blessing() {
    let s = replay(2, &[Ev::Fragments(0), Ev::EnterRegion(12), Ev::EnterRegion(0)], Policy::FloorComposed);
    assert_eq!(s.stored, 12, "floor dropping to 0 outside the DLC must never write a LOWER blessing");
    assert_eq!(s.writes, vec![12], "and it must not write at all on the way out");
}

/// A real, higher blessing the game set must never be stomped by our floor.
#[test]
fn a_higher_real_blessing_is_never_stomped() {
    let s = replay(2, &[Ev::GameSetBlessing(18), Ev::EnterRegion(12), Ev::Fragments(0)], Policy::FloorComposed);
    assert_eq!(s.stored, 18);
    assert!(s.writes.is_empty(), "already above the target -> no write at all");
}

/// A transient bag-walk failure must be INERT -- never a write, and never a flicker to 0.
#[test]
fn a_transient_bag_miss_never_writes() {
    let s = replay(2, &[Ev::Fragments(26), Ev::BagUnreadable, Ev::EnterRegion(12)], Policy::FloorComposed);
    assert_eq!(s.stored, level_for_fragments(26), "the bag miss must not disturb the standing blessing");
    assert_eq!(s.writes.len(), 1, "exactly one write (the real one); the miss tick wrote nothing");
}

/// Reconnect re-applies slot_data and re-runs the loop: idempotent, no second write.
#[test]
fn reconnect_is_idempotent() {
    let s = replay(2, &[Ev::Fragments(0), Ev::EnterRegion(12), Ev::Reconnect, Ev::Reconnect], Policy::FloorComposed);
    assert_eq!(s.stored, 12);
    assert_eq!(s.writes, vec![12], "reconnect must not re-write a level already stored");
}

/// mode 0 (off) is total: never a write, whatever happens.
#[test]
fn off_never_writes() {
    let s = replay(0, &[Ev::Fragments(50), Ev::EnterRegion(20)], Policy::FloorComposed);
...
fn mode_1_ignores_the_floor_by_design() {
    let s = replay(1, &[Ev::Fragments(0), Ev::EnterRegion(12)], Policy::FloorComposed);
...
fn blessing_target_is_max_of_fragments_and_floor_and_is_clamped() {
    assert_eq!(blessing_target(0, 50, 20), None, "off => no decision at all");
    assert_eq!(blessing_target(1, 0, 12), Some(0), "mode 1 ignores the floor");
    assert_eq!(blessing_target(2, 0, 12), Some(12), "mode 2 floors");
    assert_eq!(blessing_target(2, 26, 5), Some(level_for_fragments(26)), "fragments win when higher");
    assert_eq!(blessing_target(2, 999, 999), Some(SCADU_MAX_LEVEL), "clamped to the curve's max");
    assert_eq!(blessing_target(9, 10, 10), None, "an unknown mode must not write");
}
...
fn level_for_fragments_matches_the_vanilla_curve() {
    ...
    assert_eq!(level_for_fragments(2), 1, "below the next threshold -> stays");
    assert_eq!(level_for_fragments(50), 20, "the full set is max level");
    assert_eq!(level_for_fragments(1000), 20, "never past the top of the curve");
}
