...
fn a_primed_known_good_flag_opens_the_gate_without_writing_anything() {
    ...
    assert_eq!(s.field, START_GRACE as i32, "the field is pointed at an already-on, inert flag");
}
...
fn no_write_ever_happens_across_a_run() {
    ...
    assert!(s.flags_written.is_empty(), "not one flag written across the whole run");
}
...
fn gate_action_is_total() {
    assert_eq!(gate_action(71_002, true, 0), GateAction::AllowAndCache(71_002));
    assert_eq!(gate_action(WYRM_DEFEAT, false, 71_190), GateAction::RedirectField(71_190));
    assert_eq!(gate_action(WYRM_DEFEAT, false, 0), GateAction::Wait);
    assert_eq!(gate_action(-1, false, 0), GateAction::Wait, "a negative field is not a flag");
}
