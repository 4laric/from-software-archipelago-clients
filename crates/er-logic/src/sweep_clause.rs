...
//!
//! World-side, `gen_data` bakes ", also granted by <boss> (<tile>)" into a sweep MEMBER's
//! location name at corpus-gen time (greenfield/desc_sources.py `with_sweep`) so that a hint can
//! name the boss that hands the check over (#670). That clause describes the CORPUS -- every
...
/// The clause opener, byte-identical to greenfield/desc_sources.py `with_sweep`
/// (`f"{base}, {clause}"`). The clause always ends the descriptive text: what follows it, when
/// anything does, is the " [f<flag>]" tail the generator appends last.
const CLAUSE: &str = ", also granted by ";
...
pub fn seed_scoped_name(name: &str, ap_id: u64, active: Option<&HashSet<u64>>) -> String {
    ...
    let Some(start) = name.find(CLAUSE) else {
        return name.to_string();
    };
