...
mod tests {
    ...
    fn progressive(index: ItemIndex, name: &str, tiers: &[(&[GoodsId], &[FlagId])], overflow: GoodsId) -> ReceivedItem {
        ReceivedItem {
            ...
            semantics: ItemSemantics::Progressive {
                tiers: tiers
                    .iter()
                    .map(|(g, f)| ProgTier { goods: g.to_vec(), flags: f.to_vec(), consumed: false })
                    .collect(),
    ...
    fn progressive_mixed(
        ...
        ReceivedItem {
            ...
            semantics: ItemSemantics::Progressive {
                tiers: tiers
                    .iter()
                    .map(|(g, f, c)| ProgTier { goods: g.to_vec(), flags: f.to_vec(), consumed: *c })
                    .collect(),
                overflow_full_id: overflow,
            },
        }
    ...
    fn flask_tiers() -> Vec<(&'static [GoodsId], &'static [FlagId], bool)> {
        vec![(&[10010][..], &[71001][..], true), (&[10020][..], &[71002][..], true)]
    }
    ...
    fn consumed_tier_is_ledgered_never_a_unique_good() {
        ...
        let got: Vec<_> = d.ledgered.iter().map(|l| (l.index, l.full_id, l.qty, l.apply)).collect();
        assert_eq!(
            got,
            vec![(3, 10010, 1, true), (7, 10020, 1, true)],
            "each consumed rung is ledgered once, keyed by ITS copy's stream index"
        );
        assert_eq!(d.flags.get(&71001), Some(&true), "rung flags stay observable + self-healing");
        assert_eq!(d.flags.get(&71002), Some(&true));
        assert!(!d.owned_flags.contains(&71001), "rung flags set-only, never owned");
    }
    ...
    fn consumed_tier_grants_once_and_stays_spent_after_consumption() {
        ...
        r.run_to_fixpoint(&mut g, TickBudget::default(), 16);
        assert_eq!(g.ledger_count(10010), 1, "rung 0 granted exactly once, via the ledger");
        assert_eq!(g.ledger_count(10020), 1, "rung 1 granted exactly once, via the ledger");
    ...
    fn consumed_tier_not_regranted_on_reconnect_or_resnapshot() {
        ...
        r.run_to_fixpoint(&mut g, TickBudget::default(), 16);
        assert_eq!(g.ledger_count(10010), 1, "re-snapshot must not re-grant a consumed rung");
        assert_eq!(g.ledger_count(10020), 1);
        ...
        let mut r2 = Reconciler::from_persisted(inputs("A", items, vec![]), wm);
        r2.run_to_fixpoint(&mut g, TickBudget::default(), 16);
        assert_eq!(g.ledger_count(10010), 1, "reload must not re-grant a consumed rung");
        assert_eq!(g.ledger_count(10020), 1);
    }
    ...
    fn owned_tier_still_self_heals_after_loss() {
        ...
        r.mark_dirty();
        r.run_to_fixpoint(&mut g, TickBudget::default(), 16);
        assert!(g.has_good(8101), "an OWNED tier good must self-heal when lost (bell bearings)");
        assert!(g.ledger_log.is_empty(), "owned rungs never touch the ledger");
    }
    ...
    fn mixed_ladder_routes_each_rung_by_its_own_consumed_flag() {
        ...
        let tiers: Vec<(&[GoodsId], &[FlagId], bool)> =
            vec![(&[8101][..], &[70001][..], false), (&[10010][..], &[71001][..], true)];
        let items = vec![
        ...
        assert_eq!(d.unique_goods.len(), 1, "only the OWNED rung is a unique good");
        assert!(d.unique_goods.contains_key(&8101));
        assert_eq!(
            d.ledgered.iter().map(|l| (l.index, l.full_id)).collect::<Vec<_>>(),
            vec![(1, 10010), (2, 2919)],
        ...
        assert_eq!(g.ledger_count(10010), 1);
        assert_eq!(g.ledger_count(2919), 1, "overflow unchanged alongside consumed rungs");
    ...
    fn consumed_ladder_desired_is_order_independent() {
        ...
        assert_eq!(
            a.ledgered.iter().map(|l| (l.index, l.full_id)).collect::<Vec<_>>(),
