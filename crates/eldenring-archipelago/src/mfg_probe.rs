...
use er_logic::mfg_bridge::{ABI_VERSION, Bridge, Hover, Info, negotiate};
...
/// Only called by explicit !mfgprobe, not an opt-in to continuous tracking.
pub fn report() -> String {
    ...
    let (Some(query), Some(copy)) = (query, copy) else {
        return "mfgprobe: loaded; no complete read-only v1 API (stock builds are unsupported)"
            .to_string();
    };
    ...
    if result != 0 {
        return format!("mfgprobe: read-only query refused ({result})");
    }
    if let Err(reason) = negotiate(info) {
        return format!("mfgprobe: read-only API refused ({reason:?})");
    }
    let mut hover = Hover::default();
    let result = unsafe { copy(&mut hover, size_of::<Hover>() as u32) };
    if result != 0 {
        return format!("mfgprobe: read-only API ready; hover unavailable ({result})");
    }
