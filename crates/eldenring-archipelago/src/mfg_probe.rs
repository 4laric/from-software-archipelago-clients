...
//!
//! The inspected public MapForGoblins source contains internal visibility and highlight controls,
//! but no released build currently exports them.
...
pub fn report() -> String {
    ...
    match classify(true, version, set_state, clear) {
        Capability::ObserveOnly => format!(
            "mfgprobe: loaded; {exports}; observe-only (no stable external marker API detected)"
        ),
        Capability::IncompleteApi => format!(
            "mfgprobe: loaded; {exports}; REFUSED partial API (all v1 exports are required)"
        ),
        Capability::CompleteExportSet => format!(
            "mfgprobe: loaded; {exports}; candidate v1 export set detected (probe does not call or mutate it)"
        ),
