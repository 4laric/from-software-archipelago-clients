...
impl Game {
    ...
    /// Returns the item for `id`, falling back to a synthetic placeholder if the id
    /// isn't in this game's data package.
    ///
    /// A world that sends an id outside its own declared data package is misbehaving, but
    /// that is ITS bug and it must not cost us the packet. See [Self::location_or_placeholder].
    pub(crate) fn item_or_placeholder(&self, id: impl AsItemId) -> Item {
        let id = id.as_item_id();
        self.item(id).unwrap_or_else(|| {
            log::warn!(
                "item {id} is missing {}'s data package; using a placeholder name. The id is \
                 preserved, so delivery is unaffected.",
                self.name()
            );
            Item::new(id, format!("<item #{id}>").into(), self.name())
        })
    }
    ...
    /// Returns the location for `id`, falling back to a synthetic placeholder if the id
    /// isn't in this game's data package.
    ///
    /// 🛑 This exists because the alternative is catastrophic. `ReceivedItems` hydrates a whole
    /// batch through a single `collect::<Result<Vec<_>, _>>`, so propagating an error here
    /// discards EVERY item in the batch -- and at `index == 0` the client has already cleared
    /// its stream, leaving it permanently empty while sends keep working. One foreign world
    /// naming a location we can't resolve must not be able to do that.
    pub(crate) fn location_or_placeholder(&self, id: impl AsLocationId) -> Location {
        let id = id.as_location_id();
        self.location(id).unwrap_or_else(|| {
            log::warn!(
                "location {id} is missing {}'s data package; using a placeholder name. The id \
                 is preserved, so delivery is unaffected.",
                self.name()
            );
            Location::new(id, format!("<location #{id}>").into(), self.name())
        })
    }
